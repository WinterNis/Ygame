...
ia(Request) :- 
	cors_enable,
	getTurnInfo(Request, Board, NextPlayer, Heuristic, Depth),
	play(Board,NextPlayer,NextBoard, Heuristic, Depth),	
...
getTurnInfo(Request,Board, NextPlayer, Heuristic, Depth) :- 
	...
	http_parameters(Request, [depth(DepthString, [default(1)])]),  atom_number(DepthString, Depth).
